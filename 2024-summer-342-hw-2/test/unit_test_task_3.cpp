...
/*
 *  The root cause of the issue with this test is that formula_2 creates a shallow copy of formula 1.
 *  This results in a segmentation fault when attempting to deallocate memory since both objects share the same memory address. This also leaves the formula_2 object with dangling pointers.
 *  To resolve the memory issue, formula_2 should be a deep copy. This can be achieved by implementing a copy constructor and copy assignment operator in the formula class.
 */
